...
SELECT
    sheet2.country_name,
    MAX(sheet1.rating) AS highest_rating
FROM
    sheet1
INNER JOIN
...
SELECT country_name, restaurantname, MAX(rating) AS highest_rating, MAX(votes) AS max_votes
FROM sheet1
INNER JOIN sheet2 ON sheet1.country_code = sheet2.countryid
GROUP BY country_name, restaurantname
